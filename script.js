...
const steenBokButton = document.querySelector('.steenbok-btn');
steenBokButton.addEventListener('click', (e) => {
 const getGenderFemales = person => person.gender === 'female';
 const getAgesAboveThirthy = person => person.age >= 30;
 const days = person => parseInt(person.birthday.dmy.split("/")[0]);
 const months = person => parseInt(person.birthday.dmy.split("/")[1]);
...
const datum = randomPersonData.filter(person => person.days)
 ...
 getSteenBokFemale.forEach(person => {
     output += `  
       <div class='steenbok-card'>              
         <figure class="female-box">
             <img class="femalesteenbok-img" src="${person.photo}" alt="">
             <figcaption>${person.name} ${person.surname}</figcaption>
         </figure>
         <p>Age: ${person.age}</p>
         <p class="age-line">${person.birthday.dmy}</p>
       </div>   
     `;
   });
 conTainer.innerHTML = output;      
});
// !!!!!!=========== END BUTTON FEMALE STEENBOK 30 OR OLDER =========
...
// =========== BUTTON CREDITCARDS EXPIRATION ====================
const creditCardBtn = document.querySelector('.creditcards-btn');
creditCardBtn.addEventListener('click', (e) => {
 const conTainer = document.querySelector('.container');
 let output = '';
 const aBoveEighteen = person => person.age >= 18;
 const cardsExpired = person => person.credit_card.expiration;

 const peopleWithExpiredCreditCards = randomPersonData
 .filter(aBoveEighteen);
 peopleWithExpiredCreditCards.forEach(person => {
   output += `                
           <div class="cards">
               <figure>
                 ...
                 <div class='info-box'>
                     <p>
                       ...
                       Cardnr: ${person.credit_card.number}<br>
                       Expire: ${person.credit_card.expiration}
                     </p>
                 </div>   
           </div>  
   `;
